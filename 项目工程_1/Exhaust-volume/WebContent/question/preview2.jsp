				...
				<div id="pui_body">
					<div class="paperpart" id="paperpart1">
						<div class="partbody" id="part1">
							...
							<c:forEach items="${questionmap }" var="questionmap">
							<c:if test="${questionmap.key.name==type.name }">
								<c:if test="${fn:length(questionmap.value)==0}">
									<c:set var="id" value="${id-1 }"/>
								</c:if>
								<c:if test="${fn:length(questionmap.value)!=0}">
												...
												<tr>
													<td>
														<div class="questypetitle" style="width: auto;">
															<span class="questypeindex"> <b>${id }、</b></span> <span
																class="questypename" id="questypename1_${id }" name="${id }"
																abc="${id }"> ${type.name }（共${fn:length(questionmap.value)}小题） </span>
														</div>
													</td>
												</tr>
											</tbody>
								...
								<div class="questypebody${id }" id="questypebody_${id }">
									<ul class="dragsort-ver">
										<c:set var="sub_id" value="0"/>
										<c:forEach items="${questionmap.value }" var="map">
										<c:set var="sub_id" value="${sub_id+1 }"/>
										<li id="${map.key.id }" t_id="${sub_id }" class="ajax_li" exam_need=${sub_id },${map.key.id }>
											<c:set var="sort" value="${sort }${sub_id },${map.key.id }/"/>
											<div class="quesbox"
												...
												<div class="quesdiv">
													<font class="reportError" sub_id="4" exam_id=${map.key.id }
													...
													<div class="quesTxt quesTxt2 quesTxtfno"
														...
														<ul>
															<li class="noborder title_css"><font>答案</font> <b
																class="choiceB">${map.value.key }</b></li>
														</ul>
													</div>
												</div>
											</div>
...
<form action="${ctx }/question/saveExam" method="get">
  <div>
  	<div id="showMsg"></div>
