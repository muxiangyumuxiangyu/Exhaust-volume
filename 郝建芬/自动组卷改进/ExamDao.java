...
public class ExamDao {
	...
	public void updateYearRepeat(int yearrepeat,Set<Sort> ss) {
		Session session=sessionFactory.getCurrentSession();
		List<Integer> list=new ArrayList();
		Iterator ite = ss.iterator();
		while(ite.hasNext()){
			Sort s =(Sort)ite.next();
			System.out.println("yearrepeat¹þ¹þ¹þ"+yearrepeat);
			Question q=s.getQuestion();
			q.setRepeats(yearrepeat);
			session.update(q);
			list.add(s.getQuestion().getId());
		}
		for(int i=0;i<5;i++){
			System.out.println("lit"+list.get(i));
		}
		Query query=session.createQuery("update Question set repeats=repeats-1 where repeats>0 and id not in(:questionlist)");
		query.setParameterList("questionlist", list);
